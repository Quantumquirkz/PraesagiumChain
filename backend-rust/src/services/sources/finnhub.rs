...
impl FinnhubSource {
    ...
    pub async fn fetch_quote(&self, symbol: &str) -> Result<Signal> {
        let key = self.api_key.as_deref().ok_or_else(|| AppError::Validation("FINNHUB_API_KEY not set".into()))?;
        ...
        Ok(Signal { source: "finnhub".to_string(), price_change_24h: Some(change), volume_24h: None, sentiment: None })
    }
}
