...
- **Tables:** `market_events` (event_type, market_id, on_chain_market_id, payload JSON string), `prediction_events` (market_id, probability, uncertainty, model_version). DDL in `backend-rust/migrations_clickhouse/001_events.sql`.
