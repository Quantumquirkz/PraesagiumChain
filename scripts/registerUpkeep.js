...
async function main() {
  ...
  const registry =
    ...
    (isSepolia ? SEPOLIA_AUTOMATION_REGISTRY : null);
  const registrar =
    process.env.AUTOMATION_REGISTRAR || (isSepolia ? SEPOLIA_REGISTRAR : null);
  const linkTokenAddr =
    process.env.LINK_TOKEN || (isSepolia ? SEPOLIA_LINK_TOKEN : null);

  if (!registry || !registrar || !linkTokenAddr) {
    throw new Error(
      "Chainlink Automation addresses not configured. Set AUTOMATION_REGISTRY, AUTOMATION_REGISTRAR, LINK_TOKEN for your network.",
    );
  }
  ...
  if (balance < amount) {
    console.warn(
      `LINK balance: ${ethers.formatEther(balance)}. Need at least 1 LINK. Get from https://faucets.chain.link/`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  const registrarContract = await ethers.getContractAt(registrarAbi, registrar);
  ...
  const event = receipt.logs.find(
    (l) =>
      l.topics[0] ===
      ethers.id("UpkeepRegistered(uint256,address,uint32,address,bytes,bytes)"),
  );
